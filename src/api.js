...
export const getEvents = async() => {
    ...
    if (token) {
        ...
        const url = `https://jimo3ukqcg.execute-api.us-east-1.amazonaws.com/dev/api/get-events/${token}`;
    ...
    if (!accessToken || tokenCheck.error) {
        ...
        if (!code) {
            const results = await axios.get(
                "https://jimo3ukqcg.execute-api.us-east-1.amazonaws.com/dev/api/get-auth-url"
            );
...
const getToken = async(code) => {
    ...
    const { access_token } = await fetch(
            `https://jimo3ukqcg.execute-api.us-east-1.amazonaws.com/dev/api/token/${encodeCode}`
        )
        .then((res) => {
            return res.json();
        })
